...
namespace TestApp.MapFeatures
{
    ...
    {
        [SerializeField] private MapsService _mapsService;[Header("Materials")] [SerializeField] private Material _intersectionMaterial;
        ...
        [SerializeField] private Material _defaultBuildingRoofMaterial; private float _roadWidth = 4;[Header("Region Togglers")] [SerializeField] private bool _showExtrudedStructures = true;
        ...
        [SerializeField] private bool _showIntersections = true; public static GameObjectOptions DefaultGameObjectOptions; private RegionStyle _defaultRegionStyle;
        ...
        private ExtrudedStructureStyle _defaultExtrudedStructureStyle; private void Awake()
        {
            if (_mapsService == null)
                Debug.Log("Maps Service is required for this script to work."); _defaultRegionStyle = new RegionStyle.Builder
                {
                    FillMaterial = _grassMaterial,
                }.Build(); _defaultSegmentStyle = new SegmentStyle.Builder
                {
                    Material = _roadMaterial,
                    Width = _roadWidth,
                    BorderMaterial = _roadBorderMaterial,
                    BorderWidth = 0.6f
                }.Build(); _defaultAreaWaterStyle = new AreaWaterStyle.Builder
                {
                    FillMaterial = _waterMaterial
                }.Build(); _defaultLineWaterStyle = new LineWaterStyle.Builder
                {
                    Material = _waterMaterial
                }.Build(); _defaultModeledStructureStyle = new ModeledStructureStyle.Builder
                {
                    Material = _modeledStructureMaterial
                }.Build(); _defaultExtrudedStructureStyle = new ExtrudedStructureStyle.Builder
                {
                    WallMaterial = _defaultBuildingWallMaterial,
                    RoofMaterial = _defaultBuildingRoofMaterial
                }.Build(); DefaultGameObjectOptions = new GameObjectOptions
                {
        ...
        {
            if (_mapsService == null)
                return; _mapsService.Events.ExtrudedStructureEvents.WillCreate.AddListener(OnWillCreateExtrudedStructure);
            ...
            _mapsService.Events.IntersectionEvents.WillCreate.AddListener(OnWillCreateIntersection); _mapsService.Events.RegionEvents.DidCreate.AddListener(OnRegionCreated);
            _mapsService.Events.SegmentEvents.DidCreate.AddListener(OnSegmentCreated);
        }
        void OnDisable()
        {
            if (_mapsService == null)
                return; _mapsService.Events.ExtrudedStructureEvents.WillCreate.RemoveListener(OnWillCreateExtrudedStructure);
            ...
            _mapsService.Events.IntersectionEvents.WillCreate.RemoveListener(OnWillCreateIntersection); _mapsService.Events.RegionEvents.DidCreate.RemoveListener(OnRegionCreated);
        ...
        void OnWillCreateIntersection(WillCreateIntersectionArgs args)
        {
            if (_showIntersections)
            {
                args.Style = new SegmentStyle.Builder(args.Style)
                {
                    ...
                    Width = _roadWidth
                }.Build(); List<IntersectionArm> arms = new List<IntersectionArm>(args.MapFeature.Shape.Arms); foreach (IntersectionArm arm in arms)
                {
